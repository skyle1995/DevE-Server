...
```
+-------------+       +-------------+       +-------------+
|    admin    |       | applications |       |    cards    |
+-------------+       +-------------+       +-------------+
| id          |<----->| id          |<----->| id          |
| username    |       | name        |       | card_no     |
| password    |       | app_key     |       | type_id     |
| status      |       | status      |       | app_id      |
| create_time |       | description |       | status      |
| update_time |       | create_time |       | binding_info |
...
CREATE TABLE `app` (
  ...
  `logo_url` varchar(255) DEFAULT NULL COMMENT '应用logo URL',
  `version` varchar(20) DEFAULT NULL COMMENT '应用当前版本',
  `download_url` varchar(255) DEFAULT NULL COMMENT '应用下载地址',
  `website_url` varchar(255) DEFAULT NULL COMMENT '应用官网地址',
  ...
  `trial_hours` int(11) DEFAULT 0 COMMENT '试用小时数',
  `max_devices` int(11) DEFAULT 0 COMMENT '最大设备数，0表示不限制',
  `heartbeat` int(11) DEFAULT 0 COMMENT '心跳间隔（分钟），0表示不需要心跳',
  `allow_rebind` tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否允许换绑：0-不允许，1-允许',
  `allow_unbind` tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否允许解绑：0-不允许，1-允许',
...
CREATE TABLE `card_types` (
  ...
  `status` tinyint(1) NOT NULL DEFAULT 1 COMMENT '状态：0-禁用，1-启用',
  `max_bind_count` int(11) NOT NULL DEFAULT 0 COMMENT '最大换绑/解绑次数，0表示不限制',
  `user_id` int(11) NOT NULL COMMENT '创建者用户ID',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `cards` (
  ...
  `bind_count` int(11) NOT NULL DEFAULT 0 COMMENT '换绑/解绑次数统计',
  `user_id` int(11) NOT NULL COMMENT '创建者用户ID',
  `create_time` datetime NOT NULL COMMENT '创建时间',
...
CREATE TABLE `app_variables` (
  ...
  `type` tinyint(1) NOT NULL DEFAULT 0 COMMENT '变量类型：0-公开变量，1-私有变量',
  ...
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `idx_app_name` (`app_id`, `name`),
  KEY `idx_app_id` (`app_id`),
  KEY `idx_type` (`type`),
  KEY `idx_user_id` (`user_id`)
...
CREATE TABLE `system_settings` (
  ...
  `is_system` tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否系统设置：0-否，1-是（系统设置不可删除）',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
##### 创建应用变量
```
POST /api/panel/applications/{id}/variables
```
请求参数：
```json
{
...
##### 获取应用变量列表
```
GET /api/panel/applications/{id}/variables
```
请求参数：
```
type: 0  // 可选，变量类型：0-公开变量，1-私有变量，不传表示获取所有类型
```
响应：
```json
{
...
##### 更新应用变量
```
PUT /api/panel/applications/{app_id}/variables/{id}
```
请求参数：
```json
{
...
##### 删除应用变量
```
DELETE /api/panel/applications/{app_id}/variables/{id}
```
响应：
```json
{
